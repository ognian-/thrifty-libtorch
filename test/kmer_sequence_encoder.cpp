...
void test_encode_bases_standard() {
  auto result = netam::kmer_sequence_encoder::encode_bases("ACGT");
  netam::Assert(result.size(0) == 4);
  netam::Assert(result[0].item<int64_t>() == 0);  // A
  netam::Assert(result[1].item<int64_t>() == 1);  // C
  netam::Assert(result[2].item<int64_t>() == 2);  // G
  netam::Assert(result[3].item<int64_t>() == 3);  // T
}

void test_encode_bases_lowercase() {
  auto result = netam::kmer_sequence_encoder::encode_bases("acgt");
  netam::Assert(result.size(0) == 4);
  netam::Assert(result[0].item<int64_t>() == 0);  // a -> A
  netam::Assert(result[1].item<int64_t>() == 1);  // c -> C
  netam::Assert(result[2].item<int64_t>() == 2);  // g -> G
  netam::Assert(result[3].item<int64_t>() == 3);  // t -> T
}

void test_encode_bases_unknown() {
  auto result = netam::kmer_sequence_encoder::encode_bases("N");
  netam::Assert(result.size(0) == 1);
  netam::Assert(result[0].item<int64_t>() == 4);  // N -> 4 (unknown)
}

void test_encode_bases_mixed_with_unknown() {
  auto result = netam::kmer_sequence_encoder::encode_bases("ANCT");
  netam::Assert(result.size(0) == 4);
  netam::Assert(result[0].item<int64_t>() == 0);  // A
  netam::Assert(result[1].item<int64_t>() == 4);  // N -> unknown
  netam::Assert(result[2].item<int64_t>() == 1);  // C
  netam::Assert(result[3].item<int64_t>() == 3);  // T
}

void test_encode_bases_empty() {
  auto result = netam::kmer_sequence_encoder::encode_bases("");
  netam::Assert(result.size(0) == 0);
}
...
void test_constructor_kmer_length_3() {
  ...
  netam::kmer_sequence_encoder encoder{yaml};

  netam::Assert(encoder.kmer_length() == 3);
  netam::Assert(encoder.site_count() == 100);
  // 4^3 + 1 (for N placeholder) = 65
  netam::Assert(encoder.kmer_count() == 65);
}
...
void test_constructor_kmer_length_5() {
  ...
  netam::Assert(encoder.kmer_length() == 5);
  netam::Assert(encoder.site_count() == 200);
  // 4^5 + 1 = 1025
  netam::Assert(encoder.kmer_count() == 1025);
}
...
void test_encode_sequence_output_shape() {
  ...
  netam::Assert(encoded.size(0) == 10);
  // wt_modifier should be [site_count, 4]
  netam::Assert(wt_modifier.size(0) == 10);
  netam::Assert(wt_modifier.size(1) == 4);
}
...
void test_encode_sequence_short_sequence() {
  ...
  netam::Assert(encoded.size(0) == 100);
  netam::Assert(wt_modifier.size(0) == 100);
}
...
void test_encode_sequence_lowercase() {
  ...
  netam::Assert(torch::equal(encoded_lower, encoded_upper));
  netam::Assert(torch::equal(wt_lower, wt_upper));
}
...
void test_encode_sequence_wt_modifier_values() {
  ...
  netam::Assert(wt_modifier[0][0].item<float>() < -1e8f);
  netam::Assert(wt_modifier[0][1].item<float>() == 0.0f);
  netam::Assert(wt_modifier[0][2].item<float>() == 0.0f);
  netam::Assert(wt_modifier[0][3].item<float>() == 0.0f);

  // Position 1 has 'C' (index 1)
  netam::Assert(wt_modifier[1][0].item<float>() == 0.0f);
  netam::Assert(wt_modifier[1][1].item<float>() < -1e8f);
  netam::Assert(wt_modifier[1][2].item<float>() == 0.0f);
  netam::Assert(wt_modifier[1][3].item<float>() == 0.0f);

  // Position 2 has 'G' (index 2)
  netam::Assert(wt_modifier[2][0].item<float>() == 0.0f);
  netam::Assert(wt_modifier[2][1].item<float>() == 0.0f);
  netam::Assert(wt_modifier[2][2].item<float>() < -1e8f);
  netam::Assert(wt_modifier[2][3].item<float>() == 0.0f);

  // Positions beyond sequence length should be all zeros
  netam::Assert(wt_modifier[3][0].item<float>() == 0.0f);
  netam::Assert(wt_modifier[3][1].item<float>() == 0.0f);
  netam::Assert(wt_modifier[3][2].item<float>() == 0.0f);
  netam::Assert(wt_modifier[3][3].item<float>() == 0.0f);
}
...
void test_encode_sequence_with_n_in_middle() {
  ...
  netam::Assert(wt_modifier[2][0].item<float>() == 0.0f);
  netam::Assert(wt_modifier[2][1].item<float>() == 0.0f);
  netam::Assert(wt_modifier[2][2].item<float>() == 0.0f);
  netam::Assert(wt_modifier[2][3].item<float>() == 0.0f);
}
...
void test_encode_sequence_empty() {
  ...
  netam::Assert(torch::all(wt_modifier == 0).item<bool>());
}
...
void test_encode_sequence_single_base() {
  ...
  netam::Assert(encoded.size(0) == 5);
  // Position 0 has 'T' (index 3)
  netam::Assert(wt_modifier[0][3].item<float>() < -1e8f);
}
...
void test_encode_sequence_all_same_base() {
  ...
  netam::Assert(encoded_accessor[1] == encoded_accessor[2]);
  netam::Assert(encoded_accessor[2] == encoded_accessor[3]);
}
...
void test_encode_sequence_kmer_indices_valid() {
  ...
  for (int64_t i = 0; i < encoded.size(0); ++i) {
    netam::Assert(encoded_accessor[i] >= 0);
    netam::Assert(static_cast<std::size_t>(encoded_accessor[i]) <
                  encoder.kmer_count());
  }
}
...
void test_encode_sequence_padding_creates_zero_index() {
  ...
  netam::Assert(encoded_accessor[0] == 0);

  // Position 2: k-mer is "CGN" (contains N from padding) -> should be 0
  netam::Assert(encoded_accessor[2] == 0);
}
...
void test_encode_sequence_long_sequence_truncated() {
  ...
  netam::Assert(encoded.size(0) == 5);
  netam::Assert(wt_modifier.size(0) == 5);
}
...
int main() {
  ...
  test_encode_sequence_with_n_in_middle();
  ...
  test_encode_sequence_padding_creates_zero_index();
  test_encode_sequence_long_sequence_truncated();

  fmt::println("All kmer_sequence_encoder tests passed.");
  return 0;
}
